...
No simulator or policy needed: this loads the ``.npz`` motion clips (full-body
positions + quaternions, already in the (w,x,y,z) convention and a z-up world
frame) and records them through the generic recorder/articulation pipeline. It
runs under the normal project venv.

    python -m rl_episode_inspector.examples.humanoid.generate_demo_episodes \
        --output-dir sample_data/humanoid/episodes

Default motions come from the Isaac Lab source tree; override with --motions-dir.
"""
...
FOOT_GROUND_Z = 0.08  # foot strike threshold (m)
...
def main() -> None:
    ...
    for ep_index, clip in enumerate(clips, start=1):
        ...
        rotations = data["body_rotations"]  # (F, B, 4) wxyz
        fps = int(data["fps"])
        dt = 1.0 / fps
        ...
        for f in range(n_frames):
            ...
            # sparse footstep: any foot crossing down through the ground threshold
            strike = False
            for i in foot_idx:
                z = float(positions[f, i, 2])
                pz_prev = prev_foot_z[i]
                if pz_prev is not None and pz_prev > FOOT_GROUND_Z >= z:
                    strike = True
                prev_foot_z[i] = z
            ...
            for b, name in enumerate(body_names):
                ...
                qw, qx, qy, qz = (float(v) for v in rotations[f, b])  # already wxyz
        ...
        if episode_id:
            ...
            print(f"recorded {episode_id}: {clip}, {n_frames} frames, {n_frames * dt:.2f}s")
