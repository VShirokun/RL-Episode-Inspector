...
### Освещение сцены — `examples/scene_lights.py` + `LightSpec`
Чтобы робот был освещён **как в самой задаче**, при экспорте мешей с USD-стейджа
снимаются источники света (UsdLux: DistantLight→`directional`, DomeLight→
`hemisphere`, SphereLight и т.п.→`point`). Пишутся в `<assets>/<key>/lights.json`,
оттуда генератор эпизодов кладёт их в `ViewerSpec.lights` (модель `LightSpec`:
kind/color/intensity/direction/position; направления/позиции — в sim-фрейме z-up).
Вьювер строит из них Three.js-источники (directional/point — под `root`, чтобы
сработал маппинг z-up→y-up; ambient/hemisphere — на уровне сцены).

Важно: фотометрические единицы USD **несравнимы между типами** света
(кандела SphereLight ≠ ниты DomeLight ≠ люксы DistantLight), поэтому интенсивности
нормируются **внутри каждого типа** отдельно, а каждому типу задан разумный
real-time-таргет (иначе огромное «сырое» число одного света затирает остальные в
ноль). Простой растеризатор без IBL: dome/hemisphere взвешен сильнее (несёт
основную заливку), point — с `decay=0`.

Если у задачи света не нашлось (или экспорт без него) — вьювер использует
**дефолтный набор** (ambient + directional). Дефолтный набор всегда доступен и
включается/выключается галочкой «Default light» в шапке (`store.defaultLights`),
без пересборки сцены. Так пользователь может смотреть либо «как в задаче»
(галочка снята), либо с дополнительной нейтральной заливкой (галочка стоит).
